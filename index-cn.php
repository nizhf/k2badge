<html>
	<head>
		<title>舰队 Collection 签名生成器</title>
		<meta charset="UTF-8">
	...
	<body>
		<div id="wrapper">
			<h1>舰队 Collection 签名生成器</h1>
			<div id="language"><a href="index.php" class="button">EN</a><a href="index-jp.php" class="button">日本語</a><span class="button active">简体中文</span><a href="index-tw.php" class="button">繁體中文</a></div>
			<p>
	...
	成自定义签名图。 如遇本工具更新，请务必清理浏览器的缓存后再使用。所生成签名右侧的立绘默认为秘书舰（第一舰队旗舰）。使用中遇到程序错误或有任何建议请在<a target="_blank" href="https://github.com/Nya-chan/k2badge/issues">[这里]</a>反馈。
			...
			<div id="canvasDiv">
				...
				<div id="buttons">
					<div>
						...
						<button type="button" id="load">读取</button>
						<button type="button" id="export">将签名输出为 PNG 格式的图片</button>
					</div>
				</div>
			</div>
			<div id="tabs">
				<ul>
					<li><a href="#ttkTab">通用选项</a></li>
					<li><a href="#flagTab">舰队列表</a></li>
					<li><a href="#shipTab">改二进度 + 图纸船</a></li>
					<li><a href="#colleTab">图鉴进度海报</a></li>
					<li><a href="#furnTab">选择背景家具</a></li>
					<li><a href="#customTab">图片位置微调</a></li>
				</ul>
				<div id="ttkInfo" name="#ttkTab">
					<div>提督名称 <input type="text" name="name" maxlength="26" placeholder="無名提督"></div>
					<div>Lv. <input type="number" name="level" min="1" max="120" placeholder="1 - 120"></div>
					<div>所属服务器 <select name="server">
					...
					<div>自定义头像: <input type='file' id="avatarImg" /> <button type="button" id="avatarClear">抹去</button></div><br/>
					<div>家具<input type="checkbox" name="useBG" id="useBG" checked></div> 
					<div>显示舰队列表<input type="checkbox" name="k2" id="k2"></div>
					<div>不显示图纸船<input type="checkbox" name="useBlue" id="useBlue" checked></div> 
				</div>
			...
			<div id="furndiv" name="#furnTab">
				...
				<div id="customInputs" name="#customTab">
					...
					<hr/>
					<div>自定义背景: <input type='file' id="bgImg" /> <button type="button" id="bgClear">抹去</button></div>
					<div>背景绘横轴: <input type="number" id="bgX" min="-800" max="800" value="0"> 背景绘纵轴: <input type="number" id="bgY" min="-500" max="500" value="0"></div>
					<div>背景绘缩放: <input type="number" id="bgZ" min="10" max="500" value="100">%</div>
					<label for="bgStretch">拉伸图片?</label><input checked type="checkbox" id="bgStretch"/>
